...
@router.post("/api/stripe/charge-booking", deprecated=True)
def charge_booking(data: dict, user=Depends(get_current_user), db=Depends(get_db)):
    """DEAD ENDPOINT — kept as a 410 stub for safety.

    Audit fix (Jun 2026): grep across backend/ and app/static/js/
    showed zero callers. The booking flow charges venues via
    _create_booking_transaction in routes/gigs.py; this orphan
    duplicated that work without verifying the caller owns the
    venue. Removing the body closes a small IDOR surface (any
    authenticated user could trigger a charge against an already-
    booked venue) and prevents accidental future use.
    """
    raise HTTPException(
        410,
        "This endpoint has been removed. Booking charges are handled "
        "automatically by the booking flow (gigs.book_gig / book_slot)."
    )

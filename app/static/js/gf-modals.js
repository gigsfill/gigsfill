 ...
 *
 * All helpers render gf-modal-* classes from app/static/css/modals.css.
...
(function () {
  ...
  function _buildModal(opts) {
    ...
    body.className = 'gf-modal-body' + (opts.bodyFlush ? ' gf-modal-body--flush' : '');
    ...
    if (Array.isArray(opts.buttons) && opts.buttons.length) {
      ...
      footer.className = 'gf-modal-footer';
      opts.buttons.forEach((b) => {
        ...
        btn.onclick = () => {
          ...
          } catch (e) { console.error('[gf-modal] button onClick error:', e); }
  ...
  function _closeOne(overlay, opts) {
    ...
    overlay.classList.add('gf-modal-closing');

...
function renderCalendar() {
  ...
  //   • day 1 of every visible month (including the target month), AND
  //   • the very first grid cell (top-left) so a spillover like
  ...
  // (Jul 2026 fix: was empty blank squares). Tag with 'other-month'
  // for muted styling.
  ...
  // Trailing days into next month — fill to end-of-week so the grid
  ...
  // Wire the shared gig-hover-card on each bubble so the artist
  // profile shows the same hover preview as artist-book-gigs.html.
  // The native title="..." attributes set above act as a fallback
  // when gig-hover-card.js isn't loaded.
  if (typeof window.attachGigHoverCard === 'function') {
    document.querySelectorAll('#calGrid .cal-gig').forEach(el => {
      ...
      const g = artistGigs.filter(x => x.date === ds)[idx];
